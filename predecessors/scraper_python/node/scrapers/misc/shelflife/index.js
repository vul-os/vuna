...
const getCategories = (url) => {
  return new Promise(resp => request(url, (err, res, body) => {
    ...
    if (!err && res.statusCode == 200) {
      ...
      $('div[class="col-sm-12 nopad"]').find('li').each((index, element) => {
        item = $(element).find('a').first();
        // list.push([item.attr('href'), item.text()]);
        list.push(url + item.attr('href'));
      });
    }
    resp(list)
  }));
...
const getMaxPages = (url) => {
  return new Promise(resp => request(url, (err, res, body) => {
    ...
    if (!err && res.statusCode == 200) {
      const $ = cheerio.load(body);
      var tempPage = $('ul[class="pag_nav"]').find('li');
      page = $(tempPage[tempPage.length - 1]).find('a').text();
...
const getProductVariationData = (product) => {
