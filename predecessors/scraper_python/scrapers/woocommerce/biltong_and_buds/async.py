...
URL = 'https://www.biltongandbudz.co.za/shop/'
...
async def get_max_pages(session, url):
    try:
        ...
        async with session.get(url, headers=random_headers()) as resp:
            ...
            print(text)
            soup = BeautifulSoup(text.decode('utf-8'), 'html5lib')
            soup = soup.find("nav", {"class": "woocommerce-pagination"}).find("ul", {"class": "page-numbers nav-pagination links text-center"})
...
async def get_products_on_page(session, url):
    try:
        async with session.get(url, headers=random_headers()) as resp:
            text = await resp.read()
            ...
            soup = soup.find('main', {'id': 'main'}) \
                .find('ul', {'class': 'products row row-small large-columns-3 medium-columns-3 small-columns-2'}) \
                .findAll('li')
            if len(soup) is 0:
                return None
...
async def get_all_products(session, url):
    max_pages = await get_max_pages(session, url)
    tasks = [get_products_on_page(session, "".join([url, "page/", str(i)])) for i in range(1, int(max_pages))]
...
async def main(url, addr="localhost", port=27017):
    ...
    db = client.marajuana_sa
    conn = aiohttp.TCPConnector(limit=5)
    async with aiohttp.ClientSession(connector=conn) as session:
        while True:
            products = await get_all_products(session, url)
            result = await db.product_list.insert_one({
                "products": products,
                "date": datetime.datetime.utcnow()
            })
            # print(f'result {result.inserted_id}')
            await get_all_product_data(session, products, db)
