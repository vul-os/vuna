...
const main = async (url, mongoURL, dbName) => {
  ...
  const uniqueProducts = [...new Set(murgedProducts)];
  await saveAllItems(mongoURL, dbName, uniqueProducts);
  // // await Promise.all(murgedProducts.map(getProductData));
  uniqueProducts.forEach(function (item, index) {
    getProductData(item, mongoURL, dbName)
  });

  // await allProgress(murgedProducts.map(function(x) { return getProductData(x, mongoURL, dbName); }),
  // (p) => {
  //    console.log(`Products Bong Along Seeds = ${p.toFixed(2)} %`);
  // });
...
try {
  var URL = 'https://bongalong.co.za/shop/';
  const mongoURL = 'mongodb://localhost:27017';  
  const dbName = 'bongalong';
